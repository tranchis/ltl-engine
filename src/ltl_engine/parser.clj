...
(defmethod neg-tree->tree :TRUE [_] [:FALSE])
(defmethod neg-tree->tree :FALSE [_] [:TRUE])
...
(defmethod neg-tree->tree :U [[_ s1 s2]] [:R (neg-tree->tree s1) (neg-tree->tree s2)])
(defmethod neg-tree->tree :R [[_ s1 s2]] [:U (neg-tree->tree s1) (neg-tree->tree s2)])

(defmulti tree->tree first)
(defmethod tree->tree nil [_] [])
(defmethod tree->tree :TRUE [_] [:TRUE])
(defmethod tree->tree :FALSE [_] [:FALSE])
...
(defmethod tree->tree :I [[_ s1 s2]] [:O (neg-tree->tree s1) (tree->tree s2)])
(defmethod tree->tree :default [[k & ltls]]
  (into [] (concat [k] (into [] (map tree->tree ltls)))))
